...
const HomePage = () => {
  ...
  useEffect(() => {
    if (!checked.length || !radio.length || !radios.length || !sub.length) getAllProducts();
    // eslint-disable-next-line
  }, [checked.length, radio.length, radios.length,sub.length]);
  useEffect(() => {
    if (checked.length || radio.length || radios.length ||sub.length) filterProduct();
    // eslint-disable-next-line
    ...
    <Layout title={"All Products - Best offers "}>
      ...
      <div className="container-fluid row mt-3 home-page">
        <div className="col-md-3  filters">
          ...
          <h4 className="text-center">Filter By SubCategory</h4>
          <div className="d-flex flex-column">
            {subcategories?.map((c) => (
              <Checkbox
                key={c._id}
